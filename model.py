...
class AudioExtractor(nn.Module):
    ...
    def forward(self, x):
        # print(f"Shape before Spectrogram : {x.shape}")
...
class MobileNetV2(nn.Module):
    def __init__(self,
        ...
        if audio_extractor is not None:
            self.audio_extractor = AudioExtractor_test()
        ...
        self.coef = nn.Parameter(torch.randn(5))
    ...
    def forward(self, x : torch.Tensor):
        ...
        add_x = x0+x1+x2+x3
        # add_x = torch.einsum('bin,i->bn', torch.stack([x0, x1, x2, x3, x4], dim=1), self.coef)
        # add_x = torch.cat([x0, x1, x2, x3, x4], dim=1)
...
class AudioExtractor_test(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # self.SpecAugment = None
        self.SpecAugment = T.SpecAugment(n_time_masks=3, 
                                        time_mask_param=40, 
                                        n_freq_masks=3, 
                                        freq_mask_param=15, 
                                        iid_masks=True,
...
class XTiny_5branch(nn.Module):
    def __init__(self,
                 ...
                 margin_ratio=0,
                 *args, **kwargs):
        super(XTiny_5branch, self).__init__()
        if block is None:
            block = InvertedResidual
    ...
    def forward(self, x : torch.Tensor):
        ...
        if self.use_segmentation:
            ...
            # margin = 4 * self.margin_ratio
            # hop = slice_width - margin
            slice_width = 78 + 3 * self.margin_ratio
            margin = 4 * self.margin_ratio
            hop = slice_width - margin
            assert hop*3+slice_width == width
